...
<%
	...
	NoticeMobile n = facMobile.getNotice(Integer.parseInt(idNoticia),
			login);
	ModuleMobile m = facMobile.getModule(Integer.parseInt(idModule));
%>
<table style="width: 100%;">

	<tr>
		<td align="center" style="background-color:<%=c.getColor()%>;"><strong><%=m.getNome()%></strong></td>
	</tr>
	<tr>
	...
	<tr>
		...
		%> 		<a href="<%=request.getContextPath()%>/mobile/atividades/homeEnquete.jsp?idPoll=<%=n.getIdElement()%>&login=<%=login%>&idCourse=<%=id%>&tipo=noticiaModulo&idNotice=<%=idNoticia%>&pagina=<%=idPagina%>&idModule=<%=idModule%>">Clique
 ...
 %>
	...
	<tr>
		<td>
			<a href="<%=request.getContextPath()%>/mobile/noticias/noticiasModulo.jsp?login=<%=login%>&idCourse=<%=id%>&idModule=<%=idModule%>&pagina=<%=idPagina%>">Voltar</a>
