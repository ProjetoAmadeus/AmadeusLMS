...
<%
	...
	if (id != null && !id.equals("")) {
		m = facMobile.getModule(Integer.parseInt(id));
		noticias = m.getNoticies();
		if (noticias == null) {
			noticias = facMobile.getNoticesModule(m.getId(), login);
		}
...
<table style="width: 100%;">
	<tr>
		<td colspan="2" align="center"
			style="background-color:<%=c.getColor()%>;"><strong><%=m.getNome()%></strong></td>
	...
	<tr style="cursor: pointer;">
		<td valign="top" align="center" style="border-bottom: 1px solid #cccccc;">
			<a href="<%=request.getContextPath()%>/mobile/noticias/visualizacaoNoticiaModulo.jsp?idCourse=<%=idCourse%>&login=<%=login%>&idModule=<%=id%>&idNotice=<%=n.getId()%>&pagina=<%=paginaAtual%>"
	...
	<tr>
		...
		<%
			...
			if (tamanhoPaginacao > 1) {
				if (paginaAtual > 1) {
		%> 			<a href="<%=request.getContextPath()%>/mobile/noticias/noticiasModulo.jsp?idCourse=<%=idCourse%>&login=<%=login%>&idNotice=<%=n.getId()%>&pagina=<%=paginaAtual - 1%>">
		<< </a> &nbsp;&nbsp; <%
 	}

 		for (int l = 0; l < tamanhoPaginacao; l++) {
 			if ((l + 1) != paginaAtual) {
 %> 			<a href="<%=request.getContextPath()%>/mobile/noticias/noticiasModulo.jsp?idCourse=<%=idCourse%>&login=<%=login%>&idModule=<%=id%>&pagina=<%=(l + 1)%>"><%=l + 1%></a>
		&nbsp;&nbsp; <%
 	} else {
 %> <span><%=l + 1%></span> &nbsp;&nbsp; <%
 	}
 		...
 		if (paginaAtual < tamanhoPaginacao) {
 %> 		<a href="<%=request.getContextPath()%>/mobile/noticias/noticiasModulo.jsp?idCourse=<%=idCourse%>&login=<%=login%>&idModule=<%=id%>&pagina=<%=(paginaAtual + 1)%>">
		>> </a> <%
 	}
 	}
 %>
	...
	<tr>
		<td>
			<a href="<%=request.getContextPath()%>/mobile/curso/homeModulo.jsp?login=<%=login%>&idCourse=<%=idCourse%>&idModule=<%=id%>">Voltar</a>
