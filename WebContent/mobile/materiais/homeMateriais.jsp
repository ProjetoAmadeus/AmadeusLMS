...
<%
	...
	c = facMobile.getCourse(Integer.parseInt(curso), login);
%>
<body>
<table style="width: 100%;">
	...
	<%
		for (int i = 0; i < c.getModules().size(); i++) {
			temp = c.getModules().get(i);
			materials.addAll(temp.getMaterials());
		}
	...
	<tr>
		<td style="border-bottom: 1px solid #cccccc;" valign="top"
			align="left"><%=material.getName()%></td>
	</tr>
	<tr>
		<td align="center">
		<%
			...
			if (tamanhoPaginacao > 1) {
				if (paginaAtual > 1) {
		%> 			<a href="<%=request.getContextPath()%>/mobile/materiais/homeMateriais.jsp?idCourse=<%=curso%>&login=<%=login%>&pagina=<%=paginaAtual - 1%>">
		<< </a> &nbsp;&nbsp; <%
 	}

 		for (int l = 0; l < tamanhoPaginacao; l++) {
 %> 		<a href="<%=request.getContextPath()%>/mobile/materiais/homeMateriais.jsp?idCourse=<%=curso%>&login=<%=login%>&pagina=<%=(l + 1)%>"><%=l + 1%></a>
 ...
 %>
	...
	<tr>
		<td align="left">
			<a href="<%=request.getContextPath()%>/mobile/curso/homeCurso.jsp?idCourse=<%=c.getId()%>&login=<%=login%>">Voltar</a>
