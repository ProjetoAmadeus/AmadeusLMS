...
function answerForumActivityWithMessage(positionModule, idForum, idPerson, idMessage){
	var body = trim(dwr.util.getValue("answerBody"));

	UtilDWR.getInclude('/answerForumActivity.do?method=answerForumActivityWithMessage&answerBody='+body+'&idForum='+idForum+'&idMessage='+idMessage,
		function(data) {
			if (data.indexOf(keyUserNotLogged) != -1){
				window.open(urlUserNotLogged, "_self");
			} else if (data.indexOf(keyError) != -1){
...
function showViewNewAnswerForumActivityWithMessage(positionModule, idForum, idMessage){
	UtilDWR.getInclude('/forumActivity.do?method=showViewNewAnswerForumActivityWithMessage&idForum='+idForum+'&idMessage='+idMessage,
 		function(data) {
			dwr.util.setValue('answer'+positionModule, data, { escapeHtml:false });
  		}
  	);
