...
public class Controller {
	...
	public void integrationSocialNetworks(AccessInfo accessInfo, String twitterLogin,
		...
		try {
			PersonDAO personDAO = factory.getPersonDAO();			
				...
				person.setTwitterLogin(twitterLogin);
				person.setFacebookLogin(facebookLogin);
	...
	public void confirmRegistry(AccessInfo accinfo) {
 		String subject = "Confirmação de cadastro na Plataforma de Educação a Distância Amadeus";
 		String message = "Parabéns "+ accinfo.getPerson().getName() +",\n";
	 ...
	 */
	public MessengerMessage saveMessengerMessage(MessengerMessage message,
			Person from, Person to) {
		
		try {
			MessengerMessageDAO mmd = factory.getMessengerMessageDAO();
			message = mmd.makePersistent(message);			
		} catch (RuntimeException e) {
			throw e;
		}
		
		try {
			PersonDAO pd = factory.getPersonDAO();
			from.getSent().add(message);
			to.getReceived().add(message);			
			pd.makePersistent(from);
			pd.makePersistent(to);
		} catch (RuntimeException e) {
			throw e;
		}
	...
	public List<MessengerMessage> getAllUnreadByPerson(Person person) {
		MessengerMessageDAO mmd = factory.getMessengerMessageDAO();
		return mmd.getAllUnreadFromPerson(person);
	}
	...
	public MessengerMessage getMessengerMessageById(int idMensagem) {
		MessengerMessageDAO mmd = factory.getMessengerMessageDAO();
		MessengerMessage message = mmd.findById(idMensagem, false);
		return message;
	}

	public MessengerMessage saveOnlyMessage(MessengerMessage message) {
		MessengerMessageDAO mmd = factory.getMessengerMessageDAO();		
		return mmd.makePersistent(message);
	}
	...
	public List<MessengerMessage> getAllMessengerMessageByPerson(Person person) {
		MessengerMessageDAO mmd = factory.getMessengerMessageDAO();
		return mmd.getAllMessengerMessageByPerson(person);
	}
	...
	public int getPersonByTwitterLogin(String screenName) {
		PersonDAO personDAO = factory.getPersonDAO(); 
		return personDAO.getPersonByTwitterLogin(screenName);
	}

	public List<Tweet> getAllTweets() {
		return factory.getTweetDAO().findAll();
	}
